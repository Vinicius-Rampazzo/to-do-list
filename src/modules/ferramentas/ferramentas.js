...
function applyLanguageToolFixes(originalText, matches) {
  ...
  let lastReplacedOffset = Infinity;

  sortedMatches.forEach(match => {
    if (match.offset + match.length > lastReplacedOffset) {
      return;
    }

    if (match.replacements && match.replacements.length > 0) {
      const bestReplacement = match.replacements[0].value;
      const originalSegment = text.slice(match.offset, match.offset + match.length);

      text = text.slice(0, match.offset) + bestReplacement + text.slice(match.offset + match.length);
      lastReplacedOffset = match.offset;

      let category = 'Ortografia';
      if (match.rule && match.rule.issueType === 'punctuation') {
        category = 'Pontuação';
      } else if (match.rule && match.rule.category && match.rule.category.name) {
        category = match.rule.category.name;
      }

      changes.unshift({
        category,
        description: match.message || 'Correção sugerida pelo dicionário.',
...
function applyCustomHeuristics(text) {
  ...
  // 2. Closure terms after courtesy/content
...
function correctPunctuation(text) {
  ...
  corrected = corrected.replace(/([.,!?:;])(?=[a-zA-Zà-úÀ-Ú])/g, '$1 ');
